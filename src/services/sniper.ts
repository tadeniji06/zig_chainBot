...
class SniperService {
  ...
  async start(): Promise<void> {
    ...
    tokenMonitor.on('newToken', (event: NewTokenEvent) => this.handleNewToken(event));
    tokenMonitor.on('graduation', (event: GraduationEvent) => this.handleGraduation(event));
  ...
  private async handleGraduation(event: GraduationEvent): Promise<void> {
    ...
    for (const user of autoSnipeUsers) {
      ...
      await this.executeSnipe(user.telegram_id, event.token.denom, event.pool.poolId, settings.buy_amount_uzig);
    }
  }
  ...
  async executeSnipe(
    ...
    try {
      ...
      const balance = await zigchainService.getZigBalance(wallet.address);
      ...
      logger.error('Snipe failed', { userId, tokenDenom, error: errorMessage });

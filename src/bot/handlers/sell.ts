...
export async function handleHoldingsMenu(ctx: Context): Promise<void> {
  ...
  const holdingsList = holdings.map((h, i) => {
...
export async function handleSellMenu(ctx: Context, tokenDenom: string): Promise<void> {
  const telegramId = ctx.from?.id;
  if (!telegramId) return;
...
export async function handleSellToken(
  ...
  try {
    ...
    const tokenBalance = await zigchainService.getTokenBalance(wallet.address, tokenDenom);

    if (BigInt(tokenBalance) === BigInt(0)) {
      await ctx.editMessageText('❌ No tokens to sell (balance is 0)', {
        reply_markup: keyboards.backToMain(),
      });
      return;
    }

    const sellAmount = zigchainService.calculateBuyAmount(tokenBalance, percentage);
    const pool = await zigchainService.getPoolByToken(tokenDenom);

    if (!pool) {
      await ctx.editMessageText('❌ No liquidity pool found for this token', {
        reply_markup: keyboards.backToMain(),
      });
      return;
    }
    ...
    const slippageMultiplier = (100 - (settings?.slippage_tolerance || 5)) / 100;
    const minOutput = '1';
...
export async function handleCustomSellPercentage(ctx: Context, tokenDenom: string): Promise<void> {
  const telegramId = ctx.from?.id;
  if (!telegramId) return;

  setUserState(telegramId, { action: 'custom_sell_percentage', data: tokenDenom });

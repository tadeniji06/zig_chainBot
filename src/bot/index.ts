...
import { handleStart, handleMainMenu, handleHelp } from './handlers/start.js';
import {
  handleWalletMenu,
  handleWalletList,
  handleWalletView,
...
export function createBot(): Bot {
    ...
    await ctx.editMessageText(
      '➕ Add Wallet\n\nChoose how to add your wallet:',
      {
        reply_markup: new InlineKeyboard()
          ...
          .text('🔐 Import Private Key', 'wallet_import_key').row()
          .text('🆕 Generate New Wallet', 'wallet_generate').row()
          .text('◀️ Back', 'wallets'),
      }
    );
  });
  ...
  bot.callbackQuery('wallet_import_mnemonic', handleWalletImportMnemonic);
  ...
  bot.callbackQuery('toggle_auto_graduated', handleToggleAutoGraduated);
